...
echo '<p class="prosper_settingDesc">' . __( 'The Auto-Linker is a useful tool to link words, brands, or products to the merchant. <br><br>The Auto-Linker has two different ways to link. Either use the short code to link individual words or use the options below, which will allow you to match words that appear across multiple pages/posts.<br><br>Go to <a href="http://wordpress.prosperentdemo.com/auto-linker/">WordPress Prosperent Demo: Auto-linker</a> for more information and to see how it looks.', 'prosperent-suite' ) . '</p>';
...
if ($options['LinkAmount'])
{
	...
	for ($i = 0; $i < $options['LinkAmount']; $i++)
	{
		...
		echo '<a style="margin-left:10px; vertical-align:baseline;" onClick="deleteMyParent(this.parentNode);" class="button-secondary" href="' . admin_url( 'admin.php?page=prosperlinks_autoLinker&delete=' . $i . '&nonce='. wp_create_nonce( 'prosper_delete_setting' )) . '">' . __( 'Delete', 'prosperent-suite' ) . '</a>';
		echo '</span>';
		echo '<br class="clear" />';
	}
...
echo '<a style="margin-left:24px; vertical-align:baseline;" class="button-secondary" href="' . admin_url( 'admin.php?page=prosperlinks_autoLinker&add&nonce='. wp_create_nonce( 'prosper_add_setting' )) . '">' . __( 'Add New', 'prosperent-suite' ) . '</a>';
