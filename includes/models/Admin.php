 ...
 * @package Model
 */
class Model_Links_Admin extends Model_Links_Base
{
	/**
	...
	{
		wp_register_style( 'prospere_admin_style', PROSPERLINKS_URL . 'includes/css/admin.min.css', array(), $this->getVersion() );
	...
	public function addActionLink( $links, $file ) 
	{
		...
		if ( empty( $this_plugin ) ) 
			$this_plugin = 'prosperent-prosperLinks/prosper-links.php';

		if ( $file == $this_plugin ) 
		{
			$settings_link = '<a href="' . admin_url( 'admin.php?page=prosperlinks_general' ) . '">' . __( 'Settings', 'prosperent_suite' ) . '</a>';
	...
	public function adminHeader( $title, $form = true, $option = 'prosperent_options', $optionshort = 'prosperSuite', $contains_files = false ) 
	{
		...
		?>
		<img src="<?php echo PROSPERLINKS_IMG . '/Gears-32.png'; ?>"/><?php echo '<h1 style="margin-left:8px;display:inline-block;font-size:34px;">ProsperLinks</h1>';?>
